...
class Picotse_Block_Generator
{
    ...
    public function generate($data)
    {
        ...
        $style_content .= "}\n";
        $style_content .= "#pts-sidebar-offcanvas .mobile-navigation .has-child {\n";
        $style_content .= "    flex-wrap: wrap;\n";
        $style_content .= "    flex-direction: column;\n";
        $style_content .= "    align-items: stretch;\n";
        $style_content .= "}\n";
        $style_content .= "#pts-sidebar-offcanvas .mobile-navigation .wp-block-navigation-submenu__toggle {\n";
        $style_content .= "    display: none;\n";
        $style_content .= "}\n";
        $style_content .= "#pts-sidebar-offcanvas .mobile-navigation .has-child .wp-block-navigation__submenu-icon {\n";
        $style_content .= "    display: none;\n";
        $style_content .= "}\n";
        $style_content .= "#pts-sidebar-offcanvas .mobile-navigation .has-child .wp-block-navigation__submenu-container {\n";
        $style_content .= "    visibility: visible;\n";
        $style_content .= "    overflow: visible;\n";
        $style_content .= "    opacity: 1;\n";
        $style_content .= "    width: auto;\n";
        $style_content .= "    height: auto;\n";
        $style_content .= "    flex-basis: 100%;\n";
        $style_content .= "    position: static;\n";
        $style_content .= "    border: none;\n";
        $style_content .= "    background-color: transparent;\n";
        $style_content .= "    color: inherit;\n";
        $style_content .= "    padding-left: 1rem;\n";
        $style_content .= "    padding-top: 0;\n";
        $style_content .= "    padding-bottom: 0;\n";
        $style_content .= "}\n";
        $style_content .= "#pts-sidebar-offcanvas .mobile-navigation .has-child > .wp-block-navigation-item__content {\n";
        $style_content .= "    flex-grow: 0;\n";
        $style_content .= "}\n";
        ...
        foreach ($map as $key => $file) {
            if (! empty($selection[$key])) {
                ...
                if ('page-lp.html' === $file) {
                    ...
                    $content = $this->wrap_front_page_shell($this->get_template_main_content($file, $selection), $selection, $file);
                } else {
                    $content = $this->wrap_in_shell($this->get_template_main_content($file, $selection), $selection, $file);
                }
     ...
     *
     * @param array<string, mixed>      $selection     Wizard selection flags.
     * @param string|null               $template_file Template file name.
     */
    private function uses_two_column_layout($selection, $template_file = null)
    {
        ...
        return null !== $template_file
            && in_array($template_file, array('single-with-sidebar.html', 'page-with-sidebar.html'), true);
    }
...
<!-- /wp:html -->';
     ...
     * @return string
     */
    private function wrap_in_shell($main_inner, $selection = array(), $template_file = null)
    {
        if ($this->uses_two_column_layout($selection, $template_file)) {
     ...
     * @return string
     */
    private function wrap_front_page_shell($main_inner, $selection, $template_file = 'front-page.html')
    {
